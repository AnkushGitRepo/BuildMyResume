...
const Button = ({ children, onClick, type = "button", hideArrow = false, className, small = false }) => {
  return (
    <div className={`relative inline-flex items-center justify-center group ${className}`}>
      ...
      <button
        type={type}
        onClick={onClick}
        className={`group relative inline-flex items-center justify-center text-base rounded-xl bg-gray-900 ${small ? 'px-4 py-2' : 'px-8 py-3'} font-semibold text-white transition-all duration-200 hover:bg-gray-800 hover:shadow-lg hover:-translate-y-0.5 hover:shadow-gray-600/30`}
      >
        {children}
        {!hideArrow && (
          <svg
